...
from __future__ import (absolute_import, division, print_function,
                        unicode_literals)
from backtrader.ctp.constant import Direction
...
class CtpCommInfo(CommInfoBase):
...
class CtpBroker(with_metaclass(MetaCtpBroker, BrokerBase)):
    ...
    This class maps the orders/positions from Ctp to the
    ...
    params = (
        ('use_positions', True),
        ('commission', CtpCommInfo(mult=1.0, stocklike=False)),
    )
    ...
    def getcash(self):
        # This call cannot block if no answer is available from ctp
